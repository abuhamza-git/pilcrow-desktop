...
package com.pilcrowmd.core.domain.markdown
...
private class FrontmatterBlockParser : AbstractBlockParser() {
    ...
    override fun tryContinue(state: ParserState): BlockContinue = if (sawOpeningFence && isFence(state.line.content)) {
    ...
    override fun addLine(line: org.commonmark.parser.SourceLine) {
        ...
        body.append(line.content).append('\n')
    }
...
class FrontmatterBlockParserFactory(private val enabled: () -> Boolean) : BlockParserFactory {
    ...
    override fun tryStart(state: ParserState, matchedBlockParser: MatchedBlockParser): BlockStart? {
        ...
        val opensFrontmatter = enabled() &&
            ...
            state.indent == 0 &&
            state.line.content.subSequence(state.index, state.line.content.length).toString().trimEnd() == "---"
